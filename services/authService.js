...
const authService = {
  ...
  /**
   * Processa o código de autorização do Melhor Envio para obter e salvar os tokens.
   * Credenciais estão HARDCODED para depuração.
   * @param {string} code - O código de autorização recebido na URL de callback.
   */
  async processarCallbackMelhorEnvio(code) {
    // --- CREDENCIAIS HARDCODED PARA TESTE ---
    const ME_AUTH_URL = "https://melhorenvio.com.br/oauth/token";
    const ME_CLIENT_ID = 6472; // Como é um número, não precisa de aspas
    const ME_CLIENT_SECRET = "oLVoR0SXwI2azOMzu9nLIuJFGT5IqfYBPdAzaZ1G";
    const ME_REDIRECT_URI = "https://n8n-doodledreamsbackend.r954jc.easypanel.host/api/auth/melhor-envio/callback";
    const ME_CONTACT_EMAIL = "jvitorafb@gmail.com";
    // ----------------------------------------
    
    try {
      console.log("Processando código de autorização do Melhor Envio com dados HARDCODED...");
      console.log("Código recebido:", code);
      
      const response = await axios.post(ME_AUTH_URL, {
